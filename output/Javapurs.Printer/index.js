...
var escapeJavaString = function (s) {
    var escapeChar = function (v) {
        ...
        var $19 = code >= 32 && code <= 126;
        if ($19) {
            return Data_String_CodeUnits.singleton(v);
        };
        var hex = Data_Int.toStringAs(Data_Int.hexadecimal)(code);
        var pad = (function () {
            var $20 = Data_String_CodePoints.length(hex) === 1;
            if ($20) {
                return "000";
            };
            var $21 = Data_String_CodePoints.length(hex) === 2;
            if ($21) {
                return "00";
            };
            var $22 = Data_String_CodePoints.length(hex) === 3;
            if ($22) {
                return "0";
            };
            return "";
...
var printExpr = function (v) {
    ...
    if (v instanceof Javapurs_JavaAst.JavaAbs) {
        var $43 = Data_Array.length(v.value0) === 0;
        if ($43) {
            var bodyStr = (function () {
                if (v.value1 instanceof Javapurs_JavaAst.JavaBlock) {
                    return "{ " + (Data_String_Common.joinWith(" ")(map(printExpr)(v.value1.value0)) + (" return " + (printExpr(v.value1.value1) + "; }")));
                };
    ...
    if (v instanceof Javapurs_JavaAst.JavaWhileTrue) {
        return "(new java.util.function.Supplier<Object>() { public Object get() { " + (Data_String_Common.joinWith("")(map(function (arg) {
    ...
    if (v instanceof Javapurs_JavaAst.JavaContinue) {
        return "(new java.util.function.Supplier<Object>() { public Object get() { throw new TcoLoop(\"" + (v.value0 + ("\", new Object[]{" + (Data_String_Common.joinWith(", ")(map(printExpr)(v.value1)) + "}); } }).get()")));
    };
    ...
    if (v instanceof Javapurs_JavaAst.JavaLetRec) {
        return "(new java.util.function.Supplier<Object>() { public Object get() { " + ("class LetRecScope { " + (Data_String_Common.joinWith("")(map(function (v1) {
    ...
    if (v instanceof Javapurs_JavaAst.JavaLet) {
        ...
        var bindingsStr = Data_String_Common.joinWith(" ")(map(function (v1) {
    ...
    if (v instanceof Javapurs_JavaAst.JavaBlock) {
        return "(new java.util.function.Supplier<Object>() { public Object get() { " + (Data_String_Common.joinWith(" ")(map(printExpr)(v.value0)) + (" return " + (printExpr(v.value1) + ("; " + "} }).get()"))));
    };
    if (v instanceof Javapurs_JavaAst.JavaAssign) {
        var $113 = v.value0 === "main";
        if ($113) {
